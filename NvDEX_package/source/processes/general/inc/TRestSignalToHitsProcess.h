...
class TRestSignalToHitsProcess : public TRestEventProcess {
   ...
   protected:
    ...
    Double_t fFittingWindow;
    TVector2 fEnergyRange;    // keV
   ...
   public:
    ...
    static Double_t fitFunction4Peaks(Double_t *x, Double_t *par);
    ...
    void PrintMetadata() {
        ...
        metadata << "Fitting Window Size : " << fFittingWindow << endl;
        metadata << "Drift velocity : " << fDriftVelocity << " mm/us" << endl;
        metadata << "Signal to hits method : " << fSignalToHitMethod << endl;
        metadata << " All Deposited Energy Range : ( " << fEnergyRange.X() << ", " << fEnergyRange.Y() << " )" << endl;
